...
<div class="center">
		...
		<?php 
			$selectQuery = mysqli_query($con, "select * from cases order by id desc"); 
			...
			?>
				...
				<tr>
					<form action="" method="post">
						...
						<td>
							<a href='reportedit.php?productID=<?php echo $row['id']; ?>' type="submit" name="edit" value="Edit" style="text-decoration:none;padding:3px;border-radius:4px;border:none;background-color:steelblue;color:white">Edit</a>
...
<?php 
	...
	if (isset($_POST['delete'])) {
		...
		$checkQuery = mysqli_query($con, "select * from cases where id='$check' ");
		$noRow = mysqli_num_rows($checkQuery);

		if ($noRow>0) {
			$delete = mysqli_query($con, "delete from cases where id='$check'");
			if ($delete) {
				echo "<script>alert('Appealled  case deleted successfully'); window.open('view-report.php','_self') </script>";
			}else{
				echo "<script>alert('Appealled  case failed to delete'); window.open('view-report.php','_self') </script>";
			}
		}
	}
