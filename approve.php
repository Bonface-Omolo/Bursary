...
$selectQuery = mysqli_query($con, "select * from declaration where id='$pID' ");
...
<?php 
	...
	if (isset($_POST['edit'])) {
		...
		$checkQuery = mysqli_query($con, "select * from declaration where id='$pID' ");
		$noRow = mysqli_num_rows($checkQuery);

		if ($noRow>0){
			$updateQuery = mysqli_query($con, "update declaration set approve='$approve' where id='$pID'");
			if ($updateQuery) {
				echo "<script>alert('Bursary approved successfully'); window.open('view-application4.php','_self') </script>";
			}else{
