...
$selectQuery = mysqli_query($con, "select * from student where id='$pID' ");
...
<?php 
	...
	if (isset($_POST['edit'])) {
		...
		$checkQuery = mysqli_query($con, "select * from student where id='$pID' ");
		$noRow = mysqli_num_rows($checkQuery);

		if ($noRow>0){
			$updateQuery = mysqli_query($con, "update student set fname='$fname', lname='$lname', idno='$idno', gander='$gander', contact='$contact', email='$email' where id='$pID'");
			if ($updateQuery) {
				echo "<script>alert('Student Acount Updated successfully'); window.open('std-view.php','_self') </script>";
			}else{
