...
<body>
	...
	<div class="search">
	<form action="searchlec.php" method="post">
...
<div class="center">
		<h2 style="margin-left: 25%;">Lecturer Information</h2>
		<?php 
			$selectQuery = mysqli_query($con, "select * from lecturer order by id desc"); 
			...
			?>
				...
				<tr>
					<form action="" method="post">
						...
						<td>
							<a href='lectedit.php?productID=<?php echo $row['id']; ?>' type="submit" name="edit" value="Edit" style="text-decoration:none;padding:3px;border-radius:4px;border:none;background-color:steelblue;color:white">Edit</a>
...
<?php 
	...
	if (isset($_POST['delete'])) {
		...
		$checkQuery = mysqli_query($con, "select * from lecturer where id='$check' ");
		$noRow = mysqli_num_rows($checkQuery);

		if ($noRow>0) {
			$delete = mysqli_query($con, "delete from lecturer where id='$check'");
			if ($delete) {
				echo "<script>alert('Lecturer deleted successfully'); window.open('view-lec.php','_self') </script>";
			}else{
				echo "<script>alert('The deletion failed'); window.open('view-lec.php','_self') </script>";
			}
		}
	}
