...
<div class="center">
		...
		<?php 
			$selectQuery = mysqli_query($con, "select * from admini order by id desc"); 
...
<?php 
	...
	if (isset($_POST['delete'])) {
		...
		$checkQuery = mysqli_query($con, "select * from admini where id='$check' ");
		$noRow = mysqli_num_rows($checkQuery);

		if ($noRow>0) {
			$delete = mysqli_query($con, "delete from admini where id='$check'");
			if ($delete) {
